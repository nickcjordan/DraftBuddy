...
return a}
function Ym(a,b){R("web_all_payloads_via_jspb")&&Mh(new kj("transport.log called for JSON in JSPB only experiment"));if("log_event"===a.endpoint){Zm(a);var c=$m(a);if(R("use_new_in_memory_storage")){Vm[c]=!0;var d={cttAuthInfo:c,isJspb:!1};Wm().storePayload(d,a.payload);an(b,[],c,!1,d)}else d=Rm.get(c)||[],Rm.set(c,d),d.push(a.payload),an(b,d,c)}}
...
function dn(a,b){R("web_all_payloads_via_jspb")&&Mh(new kj("transport.logIsolatedGelPayload called in JSPB only experiment"));if("log_event"===a.endpoint){Zm(a);var c=$m(a),d=new Map;d.set(c,[a.payload]);b&&(Mm=new b);return new Of(function(e,f){Mm&&Mm.isReady()?en(d,Mm,e,f,{bypassNetworkless:!0},!0):e()})}}
...
function Xm(a,b,c){a=void 0===a?{}:a;c=void 0===c?!1:c;!c&&R("web_all_payloads_via_jspb")&&Mh(new kj("transport.flushLogs called for JSON in JSPB only experiment"));new Of(function(d,e){c?(window.clearTimeout(Pm.j),window.clearTimeout(Pm.i),Pm.i=0):(window.clearTimeout(Om.j),window.clearTimeout(Om.i),Om.i=0);if(Mm&&Mm.isReady())if(R("use_new_in_memory_storage")){var f=a,g=c,h=Mm;f=void 0===f?{}:f;g=void 0===g?!1:g;var k=new Map,l=new Map;if(void 0!==b)g?(e=Wm().extractMatchingEntries({isJspb:g,cttAuthInfo:b}),
k.set(b,e),gn(k,h,d,f)):(k=Wm().extractMatchingEntries({isJspb:g,cttAuthInfo:b}),l.set(b,k),en(l,h,d,e,f));else if(g){e=t(Object.keys(Vm));for(g=e.next();!g.done;g=e.next())l=g.value,g=Wm().extractMatchingEntries({isJspb:!0,cttAuthInfo:l}),0<g.length&&k.set(l,g),delete Vm[l];gn(k,h,d,f)}else{k=t(Object.keys(Vm));for(g=k.next();!g.done;g=k.next()){g=g.value;var m=Wm().extractMatchingEntries({isJspb:!1,cttAuthInfo:g});0<m.length&&l.set(g,m);delete Vm[g]}en(l,h,d,e,f)}}else f=a,k=c,h=Mm,f=void 0===f?
{}:f,k=void 0===k?!1:k,void 0!==b?k?(e=new Map,k=Sm.get(b)||[],e.set(b,k),gn(e,h,d,f),Sm.delete(b)):(k=new Map,l=Rm.get(b)||[],k.set(b,l),en(k,h,d,e,f),Rm.delete(b)):k?(gn(Sm,h,d,f),Sm.clear()):(en(Rm,h,d,e,f),Rm.clear());else cn(c),d()})}
...
bo[c]=b}
var bo=null;var co=window;function eo(a,b,c){this.s=this.i=this.j=null;this.l=0;this.A=!1;this.o=[];this.m=null;this.I={};if(!a)throw Error("YouTube player element ID required.");this.id=Oa(this);this.D=c;this.setup(a,b)}
...
K=y.stack||v.j||"Not available";if(K.startsWith(H+": "+z)){var M=K.split("\n");M.shift();K=M.join("\n")}var O=y.lineNumber||"Not available",ob=y.fileName||"Not available",xc=K,za=0;if(v.hasOwnProperty("args")&&v.args&&v.args.length)for(var ta=0;ta<v.args.length&&!(za=Gn(v.args[ta],"params."+ta,p,za),500<=za);ta++);else if(v.hasOwnProperty("params")&&v.params){var Z=v.params;if("object"===typeof v.params)for(q in Z){if(Z[q]){var da="params."+q,ea=In(Z[q]);p[da]=ea;za+=da.length+ea.length;if(500<za)break}}else p.params=
In(Z)}if(Nn.length)for(var Y=0;Y<Nn.length&&!(za=Gn(Nn[Y],"params.context."+Y,p,za),500<=za);Y++);navigator.vendor&&!p.hasOwnProperty("vendor")&&(p["device.vendor"]=navigator.vendor);var S={message:z,name:H,lineNumber:O,fileName:ob,stack:xc,params:p,sampleWeight:1},Sj=Number(v.columnNumber);isNaN(Sj)||(S.lineNumber=S.lineNumber+":"+Sj);if("IGNORED"===v.level)m=0;else a:{for(var Tj=Cn(),Uj=t(Tj.V),Sf=Uj.next();!Sf.done;Sf=Uj.next()){var Vj=Sf.value;if(S.message&&S.message.match(Vj.Sb)){m=Vj.weight;
...
break}}}S.params||(S.params={});var ck=Cn();S.params["params.errorServiceSignature"]="msg="+ck.V.length+"&cb="+ck.T.length;S.params["params.serviceWorker"]="false";A.document&&A.document.querySelectorAll&&(S.params["params.fscripts"]=String(document.querySelectorAll("script:not([nonce])").length));sb("sample").constructor!==rb&&(S.params["params.fconst"]="true");var va=S;window.yterr&&"function"===typeof window.yterr&&window.yterr(va);if(0!==va.sampleWeight&&!Jn.has(va.message)){if("ERROR"===h){Dn.da("handleError",
va);if(R("record_app_crashed_web")&&0===Mn&&1===va.sampleWeight)if(Mn++,R("errors_via_jspb")){var Yf=new rh;I(Yf,1,1);if(!R("report_client_error_with_app_crash_ks")){var dk=new mh;I(dk,1,va.message);var ek=new nh;J(ek,mh,3,dk);var fk=new oh;J(fk,nh,5,ek);var gk=new ph;J(gk,oh,9,fk);J(Yf,ph,4,gk)}var qo=Yf,hk=new uh;id(hk,rh,20,vh,qo);vn(hk)}else{var ik={appCrashType:"APP_CRASH_TYPE_BREAKPAD"};R("report_client_error_with_app_crash_ks")||(ik.systemHealth={crashData:{clientError:{logMessage:{message:va.message}}}});
wn("appCrashed",ik)}Ln++}else"WARNING"===h&&Dn.da("handleWarning",va);if(R("kevlar_gel_error_routing"))a:{var Zf=void 0,$f=void 0,Mc=h,Q=va;if(R("errors_via_jspb")){if(Pn())$f=void 0;else{var ac=new jh;I(ac,1,Q.stack);Q.fileName&&I(ac,4,Q.fileName);var Pa=Q.lineNumber&&Q.lineNumber.split?Q.lineNumber.split(":"):[];0!==Pa.length&&(1!==Pa.length||isNaN(Number(Pa[0]))?2!==Pa.length||isNaN(Number(Pa[0]))||isNaN(Number(Pa[1]))||(I(ac,2,Number(Pa[0])),I(ac,3,Number(Pa[1]))):I(ac,2,Number(Pa[0])));var vb=
...
va,Oc=zb.params||{},ab={urlParams:{a:"logerror",t:"jserror",type:zb.name,msg:zb.message.substr(0,250),line:zb.lineNumber,level:h,"client.name":Oc.name},postParams:{url:P("PAGE_NAME",window.location.href),file:zb.fileName},method:"POST"};Oc.version&&(ab["client.version"]=Oc.version);if(ab.postParams){zb.stack&&(ab.postParams.stack=zb.stack);for(var Ak=t(Object.keys(Oc)),lg=Ak.next();!lg.done;lg=Ak.next()){var Bk=lg.value;ab.postParams["client."+Bk]=Oc[Bk]}var mg=Gh();if(mg)for(var Ck=t(Object.keys(mg)),
ng=Ck.next();!ng.done;ng=Ck.next()){var Dk=ng.value;ab.postParams[Dk]=mg[Dk]}var Ek=P("SERVER_NAME"),Fk=P("SERVER_VERSION");Ek&&Fk&&(ab.postParams["server.name"]=Ek,ab.postParams["server.version"]=Fk)}si(P("ECATCHER_REPORT_HOST","")+"/error_204",ab)}try{Jn.add(va.message)}catch(xo){}Kn++}}}}}else throw Zb;}else console&&console.warn&&console.warn("The YouTube player is not attached to the DOM. API calls should be made after the onReady event. See more: https://developers.google.com/youtube/iframe_api_reference#Events")};function lo(a){return(0===a.search("cue")||0===a.search("load"))&&"loadModule"!==a}
